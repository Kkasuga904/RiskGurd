...
function detectPublicAdmin(line: AddedLine): RuleResult | undefined {
...
function detectK8sPrivilege(line: AddedLine): RuleResult | undefined {
...
// IAM overbroad permissions
interface WildcardLine {
  type: "action" | "resource";
  line: AddedLine;
}

function detectIam(lines: AddedLine[]): RuleResult[] {
  const actionRe = /\bAction\b\s*[:=]\s*["']?\*(:\*|:\*)?["']?/i;
  const resourceRe = /\bResource\b\s*[:=]\s*["']?\*["']?/i;
  const results: RuleResult[] = [];

  const byFile = new Map<string, WildcardLine[]>();
  ...
  for (const [file, wildcardLines] of byFile.entries()) {
    ...
    const pairedResources = new Set<number>();
    const pairedActions = new Set<number>();
    ...
    for (const action of actions) {
      if (pairedActions.has(action.line.lineNumber)) continue;
      results.push({
        file,
        line: action.line.lineNumber,
        severity: "warn",
    ...
    for (const resource of resources) {
      if (pairedResources.has(resource.line.lineNumber)) continue;
      results.push({
        file,
